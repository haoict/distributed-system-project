...
public class Server {
   ...
   *
     ...
     * the connection. There is one Listener for each Service being provided by
     * the Server. The Listener passes the Server object to the
     ...
     *
         ...
         * requests, accept them, and pass the socket on to the
         * ConnectionManager object of this server
     *
         */
        public void run() {
    ...
    /**
     * This nested class manages client connections for the server. It maintains
     * a list of current connections and enforces the maximum connection limit.
     * It creates a separate thread (one per server) that sits around and
     * wait()s to be notify()'d that a connection has terminated. When this
     * happens, it updates the list of connections.
   *
     */
     ...
     *
         ...
         * connection from a client. It either creates a Connection object for
         * the connection and adds it to the list of current connections, or, if
         * the limit on connections has been reached, it closes the connection. 
     *
         */
        ...
        /**
         * A Connection object calls this method just before it exits. This
         * method uses notify() to tell the ConnectionManager thread to wake up
         * and delete the thread that has exited.
     *
         */
   ...
   *
     ...
     *
         ...
         * method to a Listener object to the ConnectionManager.addConnection()
         * to this Connection object, and is now finally getting used to provide
         * the service. Note that just before this thread exits it calls the
         * ConnectionManager.endConnection() method to wake up the
         * ConnectionManager thread so that it can remove this Connection from
         * its list of active connections.
     *
         */
        public void run() {
            try {
